...
- **Sequence transduction** - converting one sequence of words into
...
```
    ...
    ['autumn', 'green', 'is', 'it', 'leaves', 'mean', 'spring', 'yellow']
...
We need additional things for decoders to generate a long sequence
...
```
    ...
    array([0.08, 0.16, 0.24, 1.  , 0.12, 0.12])
...
Positional encoding is then added to each value in the matrix, so the
matrix dimensions are still `(n, d_model)`.

The embedded word sequence is then processed by the attention layers and
is outputted in the same dimension i.e. `(n, d_model)`.

The de-embedding process restores the matrix dimensions to `(n, N)` and
provides a pseudo-probability for every word in the vocabulary at every
position in the sequence.

![](https://e2eml.school/images/transformers/matrix_shapes.png)

A complication of applying a softmax function after an attention mask is
that the softmax output tends to focus on a single element. In natural
language processing, we can easily imagine a single word like `it`
referring or ‘attending’ to multiple words like `red bouncy ball`.

To have multiple words in ‘mind’ when predicting the next word, we need
to simultaneously run multiple instances of attention. This component,
called **multi-head attention**, increases the computational load of
model training.

For maximal computational efficiency, **multi-head attention** also uses
matrix multiplication to project values into a lower-dimensional
embedding space:
...
- The `(n, d_model)` sequence of embedded words is the input of the
  multi-head attention layer.  
- For each head, the matrices
  ![W_v](https://latex.codecogs.com/svg.latex?W_v "W_v"),
  ![W_q](https://latex.codecogs.com/svg.latex?W_q "W_q") and
  ![W_k](https://latex.codecogs.com/svg.latex?W_k "W_k") then transform
  the embedded words into the matrices
  ![V](https://latex.codecogs.com/svg.latex?V "V"),
  ![Q](https://latex.codecogs.com/svg.latex?Q "Q") and
  ![K](https://latex.codecogs.com/svg.latex?K "K") (the values, queries
  and keys matrices).  
...
- Each head has different
  ![W_v](https://latex.codecogs.com/svg.latex?W_v "W_v"),
  ![W_q](https://latex.codecogs.com/svg.latex?W_q "W_q") and
  ![W_k](https://latex.codecogs.com/svg.latex?W_k "W_k") matrix values,
  meaning that each head can focus on a different area of the embedded
  space.  
- Following the application of attention, the outputs of each head are
  concatenated together into a larger matrix with dimensions
  `(n, h * d_v)`.  
- To output a matrix with dimensions `(n, d_model)`, the last matrix
  multiplication transformation requires a matrix with dimensions
  `(h * d_v, d_model)`.
...
- Help keep the gradient smooth. Since attention works as a filter,
