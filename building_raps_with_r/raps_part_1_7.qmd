...
3. Render the report template using a separate R script. The code below uses `lapply()` to apply the same `rmarkdown::render()` function for each continuous variable from the iris data set. The output files can be accessed [from this subdirectory](./output).      
...
+ [An in-depth tutorial on creating parameterised reports using `purrr::map()`](https://book.rwithoutstatistics.com/parameterized-reports-chapter.html)   