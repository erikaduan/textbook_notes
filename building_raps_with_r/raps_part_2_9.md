...
- Removal of exploratory code to check specific locality spelling
  examples.  
- An extra raw data set filtering step to remove rows where the locality
  contains the word “Source” i.e. `filter(!grepl("Source", locality))`.
...
The next step of the original code is below. This code step is omitted
in the revised code as the revised code directly filters out rows where
the locality contains the word “Source”. The revised code also generates
a separate country level data set at a later stage of its workflow.
...
- The task of extracting current and former communes has been refactored
  as two different functions.  
- The task of comparing our data set communes to the Wikipedia reference
  list of all communes has been refactored as a single large function.  
- Add a comment to describe the expected output from our final code test
  `setdiff(flat_data$locality, communes)`.  
- The tasks of extracting the country versus commune level datas set
  have been refactored into separate functions.
