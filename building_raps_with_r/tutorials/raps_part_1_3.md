...
The data analysis project is on housing in Luxembourg. Its components
involve:
...
- Confirming that house prices are correctly extracted for all communes
  and cantons and at a national level.  
...
- I found it useful to manually break up single scripts into logical
  chunks using `---` separators
  i.e. `step 1: extract the data and convert from Excel to csv`.  
- I found it useful to keep and highlight functions at the top of the
  script i.e. for the `read_clean()` function. Comments that explain the
  quirks of the raw data set, downstream data analysis requirements and
  other hidden assumptions unknown to readers are the most helpful.
  Comments that describe what the code does are least helpful if the
  function names are already easy to read and clearly describe function
  intent.  
- The script contains a mix of code that performs an action versus code
  that displays a data output. There should be a neater method of
  handling this mix of code purposes (for example, by displaying and
  reviewing data outputs in separate unit tests).  
- Data analytical code relies on hard-coded assumptions, for example to
  ...
  standardise the spelling of locations (this was the most finicky to
  decode/debug). What are optimal methods of handling hard-coded
  assumptions? Documenting them inside custom functions?  
- Data analytical code relies on assumptions that the format of source
  raw data sets from the internet remains consistent. Unit tests
  confirming these undocumented assumptions are important.  
- The `raw_data` object was originally overwritten several times in the
  script. I prefer workflows where data objects are all given unique
  names and never overwritten.  
...
- For true reproducibility, we should create a snapshot of the original
  computational environment and make this available to other users.
