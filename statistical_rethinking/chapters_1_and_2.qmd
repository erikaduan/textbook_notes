...
Bayesian statistics and probability theory share one thing in common: in order to find the best possible answer (the answer that occurs the most often), **it helps to count every possibility of how something could have happened**.      

![](../figures/sr_chapter_2_counting_events.svg)
The approach above requires:    

  - defining a generative model of the sample (mapping all the possible ways that sample generation occurs i.e. drawing all the possible forking paths)          
  - defining all the possible scenarios, to compare how likely they are to have occurred (similar to designing a specific estimand and describing the range of values the estimand may take)    
  - designing a statistical way to produce an estimate (there must be a statistical method of testing your generative model).    
  
With this approach, the emphasis is on identifying a sound data generation process (or process model) and applying statistical tests against the model to identify the most likely scenario.    